...
# ---------------------------------------------------------
# KONEKSI DATABASE & FUNGSI UTILITY
# ---------------------------------------------------------
def get_connection():
    try:
        return mysql.connector.connect(
            host="localhost",
            user="root",
            password="",  # Sesuaikan password MySQL XAMPP kamu
...
# Fungsi untuk enkripsi password (SHA-256)
...
if not st.session_state.logged_in:
    st.markdown("<h1 style='text-align: center;'>Rentalinaja</h1>", unsafe_allow_html=True)
    
    # Tab Navigasi antara Login dan Register
                ...
                if btn_login:
                    if username_input and password_input:
                        ...
                        if conn:
                            ...
                            if result and check_hashes(password_input, result[0]):
                    ...
                    if new_username and new_password:
                        if new_password != confirm_password:
                            ...
                            if conn:
                                ...
                                try:
                                    cursor.execute(query, (new_username, hashed_pw))
                                    conn.commit()
                                    st.success("Akun berhasil dibuat! Silakan pindah ke tab Login.")
                                except mysql.connector.Error as err:
                                    if err.errno == 1062:  # Duplicate entry error code
                                        st.error("Username sudah terpakai, silakan gunakan username lain.")
                                    else:
                                        st.error(f"Gagal mendaftar: {err}")
                                finally:
                                    cursor.close()
                                    conn.close()
                    else:
                ...
                with st.expander("Hapus Data Penyewa"):
                    ...
                    if st.button("Hapus Penyewa", type="primary"):
                        ...
                        try:
                            ...
                            st.error(f"Gagal menghapus! Data ini mungkin terkait transaksi: {err}")
                        finally:
                            cursor.close()
            conn.close()
